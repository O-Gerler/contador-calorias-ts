import React, { useMemo } from "react";
import { TypeActivity } from "../types";
import { categories } from "../data/categories";
import { PencilSquareIcon, XCircleIcon } from "@heroicons/react/24/outline"
import { TypeActivityActions } from "../reducer/activityReducer";

type TypeActivityListProps = {
  activities: TypeActivity[];
  dispatch: React.Dispatch<TypeActivityActions>
};

export default function ActivityList({ activities, dispatch }: TypeActivityListProps) {
  const categoryName = useMemo(
    () => (category: TypeActivity["category"]) => {
      return categories.map((cat) => (cat.id === category ? cat.name : ""));
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [activities]
  );

  const isEmptyActivities = useMemo(() => activities.length === 0, [activities])

  const handleUpdate = (activityId : TypeActivity["id"]) => {
    dispatch({ type: "set-activeId", payload: { id: activityId } })
  }

  const handleDelete = (activityId : TypeActivity["id"]) => {
    dispatch({ type: "delete-activity", payload: { id: activityId } })
  }

  return (
    <section className="p-10 mx-auto max-w-4xl">
      <h2 className="text-4xl font-bold text-slate-600 text-center">
        Comida y Actividades
      </h2>
      {isEmptyActivities ? (
        <p className="text-center mt-5 text-lg italic font-thin">Aun no has guardado nada por aqui</p>
      ) : (
        activities.map((activity) => (
          <div
            key={activity.id}
            className="px-5 py-10 bg-white mt-5 flex justify-between shadow"
          >
            <div className="space-y-2 relative">
              <p
                className={`absolute -top-8 -left-8 py-2 px-10 text-white uppercase font-bold ${
                  activity.category === 1 ? "bg-lime-500" : "bg-orange-500"
                }`}
              >
                {categoryName(+activity.category)}
              </p>
              <p className="text-2xl font-bold pt-5">{activity.name}</p>
              <p className="font-black text-4xl text-lime-500">
                {activity.calories} <span>Calorias</span>
              </p>
            </div>
            <div className="flex gap-5 items-center">
              <button onClick={() => handleUpdate(activity.id)}>
                <PencilSquareIcon className="h-8 w-8 text-gray-800" />
              </button>
              <button onClick={() => handleDelete(activity.id)}>
                <XCircleIcon className="h-8 w-8 text-red-500" />
              </button>
            </div>
          </div>
        ))
      )}
    </section>
  );
}
